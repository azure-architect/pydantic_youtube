# ollama_toolkit/tools/text_segmentation.py
from typing import List, Dict, Any, Optional
from pydantic import BaseModel
import logging
from ..function_calling import call_with_function
from ..context import calculate_optimal_ctx_size, split_long_text

class TopicList(BaseModel):
    """Model for list of identified topics in a transcript"""
    sections: List[str]

class TranscriptSegment(BaseModel):
    """Model for a segment of transcript text"""
    topic: str
    content: str
...
def segment_transcript(
    ...
    topics_prompt = f"""Read this YouTube transcript and identify {max_topics-2}-{max_topics} main sections.
...
def detect_and_resolve_overlaps(segments: List[TranscriptSegment]) -> List[TranscriptSegment]:
    ...
    for i in range(len(segments)):
        for j in range(i+1, len(segments)):
            ...
            words_i = set(segments[i].content.lower().split())
            words_j = set(segments[j].content.lower().split())
